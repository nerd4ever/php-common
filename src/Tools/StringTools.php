...
class StringTools
{
    ...
    static public function stripAccents($text): string
    {
    ...
    static public function convertFileSize($bytes, $decimals): string
    {
    ...
    static public function toNamespace(string $text): string
    {
        $r = preg_replace("/[^A-Za-z ]/", '', self::stripAccents($text));
        $r2 = strtolower($r);
        return str_replace(' ', '-', $r2);
    }

    static public function tokenizeLetter(string $text): string
    {
        $r = preg_replace("/[^A-Za-z]/", '', self::stripAccents($text));
        $r2 = strtolower($r);
        return md5($r2);
    ...
    public static function fixDirectorySeparator($filename, $separator = null)
    {
        if ($separator != null) {
            $output = str_replace('\\', $separator, str_replace('\\', '/', $filename));
        } else {
            $output = (DIRECTORY_SEPARATOR === '\\') ? str_replace('/', '\\', $filename) : str_replace('\\', '/', $filename);
        }
        return $output;
    }
